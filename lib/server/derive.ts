import { ROLE_CONFIGS_FULL_V3 } from './roleConfigs_full_v3';
...
function pickRawFeaturesV1(input: any): RawFeaturesV1 {
  ...
  return {
    units: (() => {
      const u = rf?.layer_0?.units;
      if (u != null) return numOr0(u);
      // Fallbacks for stripped fixtures
      const ul = rf?.layer_0?.unit_lengths;
      if (Array.isArray(ul) && ul.length) return Math.max(1, ul.length);
      const t = rf?.layer_0?.per_unit?.transitions;
      if (Array.isArray(t) && t.length) return Math.max(1, t.length);
      return 0;
    })(),
    claims: numOr0(rf?.layer_0?.claims),
    reasons: (() => {
      const r = rf?.layer_0?.reasons;
      if (r != null) return numOr0(r);
      // Some fixtures omit reasons; no reliable reconstruction exists, default 0.
      return 0;
    })(),
    evidence: evidence,
